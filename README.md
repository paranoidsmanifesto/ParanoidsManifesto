...
Hardware backdoors exist in many CPUs today. Any CPU manufactured by Intel after the year of 2008 can be considered compromised due to their "security technologies" that they have implemented inside of the CPU. This includes their Management Engine (ME), which allows for remote administration and (signed) code execution directly on the CPU itself. Since the CPU is very low-level, it can be considered a Ring 0 backdoor, and will have access to virtually every device and its firmware. The Intel ME can be utilized while the device is turned off. In a hypothetical situation where the signing key for their remote code execution backdoor is compromised (knowingly or even unknowingly), that would allow an attacker to create a botnet of billions of devices. This also applies to AMD, which is a competitor to Intel. AMD has been integrating their AMD Secure Processor technology into their CPU's since 2014. This technology is similar to Intel's ME. These CPU backdoors can be mitigated, either by disabling it, or fully removing it from the system. This is dependent on the CPU. Newer CPU's cannot have it fully neutralized, as it is part of the boot process, and run as a completely-separate environment with totalitarian access to the system and its actions. Anything is possible with this type of access.

Hardware backdoors are not only present on CPUs, and also exist in other devices as well. Many modern smartphones that contain a modem used to connect to cellular towers contain a system on chip, and have a full environment on that chip. Through vulnerabilites (which have already been found), an attacker will be able to compromise the modem and access any data on the system.

todo: ~~Firmware blobs~~, ~~hardware backdoors~~, phones, computers, operating systems
